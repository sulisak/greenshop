...
//        (SELECT SUM(sd.product_price * sd.product_sale_num)
//         FROM sale_list_datail as sd
//         WHERE sd.owner_id = 1 AND sd.sale_runno="'.$data['sale_runno'].'") as totalprice,
// FROM sale_list_datail as sd
// LEFT JOIN wh_product_list as wl ON wl.product_id = sd.product_id
// WHERE sd.owner_id="'.$_SESSION['owner_id'].'"  AND sd.sale_runno="'.$data['sale_runno'].'"
// GROUP BY sd.sale_runno, sd.owner_id
// ORDER BY sd.sale_runno ASC');
// $encode_data = json_encode($query->result(),JSON_UNESCAPED_UNICODE );
// return $encode_data;
